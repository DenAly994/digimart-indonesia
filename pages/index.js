...
import { 
    ...
    Database, List, Trash2, Smartphone, Key, Settings, 
    ArrowRight, Plus, LayoutDashboard, Package, Server, Zap 
} from 'lucide-react';
...
export default function SmartIndex() {
    ...
    useEffect(() => {
        const unsubInv = onSnapshot(collection(db, 'inventory'), (snap) => {
            setProducts(snap.docs.map(d => ({ id: d.id, ...d.data() })).sort((a,b) => a.price - b.price));
        });
    ...
    const handleClaim = async () => {
        ...
        try {
            ...
            if (!orderSnap.exists()) { setLoadingAction(false); return alert("ACCESS DENIED: Invoice tidak ditemukan dalam matriks."); }
            if (orderSnap.data().status !== 'paid') { setLoadingAction(false); return alert("ACCESS DENIED: Otorisasi pembayaran belum selesai."); }
            if (orderSnap.data().claimed === true) { setLoadingAction(false); return alert("ACCESS DENIED: Invoice telah digunakan sebelumnya."); }
            ...
            if(!snap.empty) {
                ...
                const batch = writeBatch(db);
                batch.update(doc(db, 'accounts', acc.id), { status: 'in_use', currentUser: user.email, invoiceRef: invoice, claimedAt: new Date() });
                batch.update(orderRef, { claimed: true, claimedBy: user.email, accountId: acc.id, claimedAt: new Date() });
                await batch.commit();
                setAccessData(acc.data());
            } else { alert("SYSTEM BUSY: Semua node penuh. Silakan masuk dalam antrean."); }
        } catch (error) { alert("Sistem Database sibuk. Coba lagi."); }
        setLoadingAction(false);
    };
    ...
    // ==========================================
    // RENDER: TAMPILAN ADMIN (COMMANDER CENTER)
    // ==========================================
    if (isAdmin) {
        return (
                            ...
                            {activeMenu === 'nodes' && (
                                <div style={s.nodesGrid}>
                                    <div style={s.cardAdminLg}>
                                        ...
                                        <p style={{fontSize:'12px', color:'#666', marginBottom:'15px'}}>Masukkan data raw dengan format <code style={{color:'#3b82f6'}}>username|password</code> per baris.</p>
    ...
    // ==========================================
    // RENDER: TAMPILAN PEMBELI (STOREFRONT UI)
    // ==========================================
    return (
        <div style={s.storeContainer}>
            ...
            <nav style={s.storeNav}>
                ...
                {user ? (
                    <motion.div style={{display:'flex', alignItems:'center', gap:'15px'}}>
                ...
                <motion.div variants={stagger} initial="hidden" animate="visible" style={s.gridStore}>
                    {products.length === 0 ? <p style={{textAlign:'center', width:'100%', opacity:0.3, gridColumn:'1/-1'}}>Protokol tidak tersedia saat ini.</p> : products.map(p => (
                        <motion.div variants={fadeUp} key={p.id} style={s.cardStore}>
                            <div style={s.cardTop}>
                                <div style={s.tagStore}>{p.name.split('/')[1] || 'SESS'}</div>
                            ...
                            <div style={s.priceBox}>
                                ...
                                <span style={s.priceText}>{p.price?.toLocaleString('id-ID')}</span>
                            </div>
            ...
            {/* MODAL KLAIM PEMBELI (TERMINAL UI) */}
            <AnimatePresence>
                {showClaim && (
                            ...
                            <div style={s.modalBody}>
                                {!user ? (
                                    <div style={{textAlign:'center', padding:'20px 0'}}>
                                        <div style={{width:'50px', height:'50px', borderRadius:'50%', background:'rgba(255,255,255,0.05)', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 20px auto'}}><Lock size={20} color="#666"/></div>
                                    ...
                                    <>
                                        ...
                                        <p style={{fontSize:'10px', color:'#555', textAlign:'center', marginTop:'15px', display:'flex', alignItems:'center', justifyContent:'center', gap:'5px'}}>
                                            <Lock size={10}/> Data dilindungi oleh protokol verifikasi otomatis.
                                        </p>
                                    </>
                                )}
                            </div>
